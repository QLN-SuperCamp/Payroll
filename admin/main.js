...
var table5CAHourly = document.getElementById('table-payPeriod5-CA-hourly');
var table5CASalary = document.getElementById('table-payPeriod5-CA-salary');
...
var body5CAHourly = document.getElementById('body-payPeriod5-CA-hourly');
var body5CASalary = document.getElementById('body-payPeriod5-CA-salary');
...
$.get(report, function(data) {
	...
	reportArray.data.forEach(function(staffMember) {
		...
		let fullName = staffMember['First Name'].concat(
			' ',
			staffMember['Last Name']
		);
...
function init() {
	var publicSpreadsheetUrl =
		'https://docs.google.com/spreadsheets/d/1SbGhrkqAb8Ej4mFn9HxcfbVlK6B0zhA_2eQLB7UfnU8/edit?usp=sharing';
...
function displayData() {
			...
			function countHours(positionArray, positionType) {
				if (positionArray !== undefined) {
					...
					positionArray.forEach(function(person) {
						...
						var correspondingPersonObject = reportArray.data.find(function(
							object
						) {
							return object.fullName === fullName;
						});
						var correspondingPayPeriodArray =
							correspondingPersonObject[`${payPeriod}`];
						var week1Array = correspondingPayPeriodArray[0];
						var week2Array = correspondingPayPeriodArray[1];

						// Info objects
						var correspondingPayPeriodInfoObject =
							correspondingPayPeriodArray[2][0];
						...
						if (
							week1Array.find(function(object) {
								return object.Day === entryDate;
							}) !== undefined
						) {
							inWeek1 = true;
						} else if (
							week2Array.find(function(object) {
								return object.Day === entryDate;
							}) !== undefined
						) {
							inWeek2 = true;
						...
						if (inWeek1 === true) {
							...
							if (hoursType > 0) {
								daysInARow++;
								hoursObject = splitHours(
									site,
									hoursType,
									totalHoursThisWeek,
									daysInARow,
									correspondingPersonObject,
									correspondingDateObject
								);
						...
						correspondingPayPeriodInfoObject.totalHours =
							week1InfoObject.totalHours + week2InfoObject.totalHours;
						correspondingPayPeriodInfoObject.regHours =
							week1InfoObject.regHours + week2InfoObject.regHours;
						correspondingPayPeriodInfoObject.otHours =
							week1InfoObject.otHours + week2InfoObject.otHours;
						correspondingPayPeriodInfoObject.dtHours =
							week1InfoObject.dtHours + week2InfoObject.dtHours;
						correspondingPayPeriodInfoObject.sickHours =
							week1InfoObject.sickHours + week2InfoObject.sickHours;
			...
			function countExceptionHours(positionArray) {
				if (positionArray.length > 0) {
					...
					positionArray.forEach(function(person) {
						...
						let correspondingPersonObject = reportArray.data.find(function(
							object
						) {
							return object.fullName === name;
						});
						let correspondingPayPeriodArray =
							correspondingPersonObject[`${payPeriod}`];
						let week1Array = correspondingPayPeriodArray[0];
						let week2Array = correspondingPayPeriodArray[1];

						// Info objects
						let correspondingPayPeriodInfoObject =
							correspondingPayPeriodArray[2][0];
	...
	reportArray.data.forEach(function(person) {
		...
		let deductionDayFields = [
			person.PayPeriod1DeductionDays,
			person.PayPeriod2DeductionDays,
			person.PayPeriod3DeductionDays,
			person.PayPeriod4DeductionDays,
			person.PayPeriod5DeductionDays
		];
		let deductionWeekFields = [
			person.PayPeriod1DeductionWeeks,
			person.PayPeriod2DeductionWeeks,
			person.PayPeriod3DeductionWeeks,
			person.PayPeriod4DeductionWeeks,
			person.PayPeriod5DeductionWeeks
		];
		let checkLocationFields = [
			person.PayPeriod1Location,
			person.PayPeriod2Location,
			person.PayPeriod3Location,
			person.PayPeriod4Location,
			person.PayPeriod5Location
		];
		let salaryDayFields = [
			person.PayPeriod1SalaryDays,
			person.PayPeriod2SalaryDays,
			person.PayPeriod3SalaryDays,
			person.PayPeriod4SalaryDays,
			person.PayPeriod5SalaryDays
		];
		let salaryWeekFields = [
			person.PayPeriod1SalaryWeeks,
			person.PayPeriod2SalaryWeeks,
			person.PayPeriod3SalaryWeeks,
			person.PayPeriod4SalaryWeeks,
			person.PayPeriod5SalaryWeeks
		];
		let isSalaryFields = [
			person.PayPeriod1isSalary,
			person.PayPeriod2isSalary,
			person.PayPeriod3isSalary,
			person.PayPeriod4isSalary,
			person.PayPeriod5isSalary
		];
		...
		payPeriods.forEach(function(payPeriod) {
			...
			if (site === 'ucf') {
				...
				deductionWeekRate = 105.0;
			} else if (site === 'stan') {
				deductionDayRate = 16.76;
				deductionWeekRate = 160.0;
			} else if (site === 'villa') {
				deductionDayRate = 11.06;
				deductionWeekRate = 105.0;
			}
			// Assign a string for the deduction
			let deductionTotal = moneyString(
				deductionDayRate * deductionDays + deductionWeekRate * deductionWeeks
			);
			...
			if (isSalaryEmployee === 'Yes') {
				...
				let salaryDayPay = checkIfRound(
					salaryDays,
					dailyPayRate,
					dailyPayRateUnRounded
				);
...
function splitHours(
	...
	if (
		peopleObjects.find(
			object => object.id === correspondingPersonObject.fullName
		)
	) {
		const checkObj = peopleObjects.find(
			object => object.id === correspondingPersonObject.fullName
		);
 ...
 */
function createTableEntryHourly(
	table,
	firstName,
	lastName,
	regHours,
	...
	if (stlStipend.find(object => object.name === fullName)) {
		if (
			payPeriodNumber ===
			stlStipend.find(object => object.name === fullName).finalPayPeriod
		) {
 ...
 * @param {string} checkLocation - The location where the check/paystub should be mailed. Either "Southwestern", "Stanford", "UCF", "Villanova", or "Home".
 */
function createTableEntrySalary(
	table,
	firstName,
	lastName,
	salaryPayTotal,
	...
	const correspondingStipendObject = travelStipends.find(
		object => object.name === fullName
	);
	...
	const correspondingSpecialContract = specialContracts.find(
		object => object.name === fullName
	);
	let correspondingTotalPay = '';
	let correspondingPayPeriodObject = '';
	// If the person is in the array of weird contracts
	if (correspondingSpecialContract !== undefined) {
		if (
			correspondingSpecialContract.payPeriods.find(
				object => object.payPeriodNumber === payPeriodNumber
			)
		) {
			correspondingPayPeriodObject = correspondingSpecialContract.payPeriods.find(
				object => object.payPeriodNumber === payPeriodNumber
			);
		}

		if (correspondingPayPeriodObject !== '') {
			correspondingTotalPay = correspondingSpecialContract.payPeriods.find(
				object => object.payPeriodNumber === payPeriodNumber
			).totalPay;
		}
	}
...
function constructTables() {
	...
	$('#table-payPeriod5-CA-salary').DataTable({
		dom: 'Bfrtip',
		buttons: ['copy', 'excel', 'pdf']
	});
}
