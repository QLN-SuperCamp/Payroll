...
Cypress.Commands.add("fillSiteInfoBecca", (site, camp, month, day) => {
    cy.visit('/index.html');
    cy.get('#firstName').type("Kay", {force: true});
    cy.get('#lastName').type("Routh", {force: true});
    cy.get(`#${site}`).check({
        force: true
    });
    cy.get(`#${camp}`).check({
...
*/
context("Generate UCF Data", function () {
    var tlsToUncheck = [];
    ...
    context("University of Central Florida", function () {
